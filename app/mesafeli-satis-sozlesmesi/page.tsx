export default function DistanceSalesPage() {
  return (
    <section className="legal-page">
      <h1>Mesafeli Satış Sözleşmesi</h1>
      <p>
        Bu metin, 6502 sayılı Tüketicinin Korunması Hakkında Kanun ve Mesafeli
        Sözleşmeler Yönetmeliği çerçevesinde dijital dilekçe hazırlama hizmeti
        için hazırlanmıştır.
      </p>

      <h2>1. Taraflar</h2>
      <p>Satıcı: Ahmet Selim Baltaş</p>
      <p>Adres: Fatih Mah. Sümer Cad. No: 9 Sultanbeyli / İstanbul</p>
      <p>E-posta: info@itirazdilekcesi.com</p>
      <p>Telefon: 0506 102 14 01</p>
      <p>Alıcı: Sipariş formunu dolduran kullanıcıdır.</p>

      <h2>2. Sözleşmenin Konusu</h2>
      <p>
        İşbu sözleşmenin konusu, alıcının elektronik ortamda sipariş verdiği
        “Dijital Dilekçe Hazırlama” hizmetinin sunulmasına ilişkin tarafların hak
        ve yükümlülüklerinin belirlenmesidir.
      </p>

      <h2>3. Hizmetin Kapsamı</h2>
      <p>
        Hizmet, alıcının sisteme girdiği bilgiler doğrultusunda dijital ortamda
        örnek dilekçe metni ve PDF çıktısı oluşturulmasını kapsar. Hizmet
        avukatlık veya hukuki temsil hizmeti değildir.
      </p>

      <h2>4. Ücret ve Ödeme</h2>
      <p>
        Hizmet bedeli ödeme ekranında belirtilen 19,99 TL tutardır. Ödeme,
        iyzico veya entegre ödeme altyapısı üzerinden tahsil edilir. Ödeme
        onayı sonrası dijital içerik alıcıya anında sunulur.
      </p>

      <h2>5. Teslimat</h2>
      <p>
        Dijital dilekçe, ödeme onayından sonra elektronik ortamda derhal
        erişime açılır ve PDF olarak indirilebilir hale gelir.
      </p>

      <h2>6. Genel Hükümler</h2>
      <p>
        Alıcı, sipariş oluşturmadan önce ön bilgilendirme formunu ve işbu
        sözleşmeyi okuyup kabul ettiğini; sisteme girilen bilgilerin doğru ve
        kendisine ait olduğunu beyan eder.
      </p>

      <h2>7. Uyuşmazlıkların Çözümü</h2>
      <p>
        İşbu sözleşmeden doğabilecek uyuşmazlıklarda, ilgili mevzuat uyarınca
        tüketici hakem heyetleri ve tüketici mahkemeleri yetkilidir.
      </p>
    </section>
  );
}
