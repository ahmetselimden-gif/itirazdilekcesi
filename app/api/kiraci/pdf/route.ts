...
function generateDocumentNumber(): string {
  ...
  return `KRC-${timestamp.toString().slice(-6)}-${random.toString().padStart(4, '0')}`;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Counterparty info
    doc.fontSize(11).font('Helvetica-Bold').text('MUHATAP (KARŞI TARAF) BİLGİLERİ');
    doc.fontSize(10).font('Helvetica');
    ...
    doc.fontSize(10).font('Helvetica').text(`${body.problemType} nedeniyle kiracı tarafından itiraz dilekçesi`);
    doc.moveDown(0.8);
    ...
    doc.font('Helvetica').text(
      `${body.problemType} konusu nedeniyle önemli uyuşmazlık ortaya çıkmıştır. Belirtilen konu, kiracılık ilişkisinin devamı ve korunması açısından hayati önem taşımaktadır.`,
      { align: 'justify' }
    );
